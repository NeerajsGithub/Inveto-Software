...
class _SelectRackPageState extends State<SelectRackPage> {
  ...
  Widget rackManage(String alphabet, BuildContext context, Map<String, dynamic> store) {
    ...
    for (int i = 1; i <= 6; i++) {
      ...
      for (int j = 1; j <= 4; j++) {
        String label = alphabet + i.toString() + j.toString();
         bool isSelected = selectedRack == label;
         if (store['data'][alphabet.toString()] != null &&
          store['data'][alphabet.toString()].where((rack) => rack['rack'] == label && rack['status'] == 'available').isNotEmpty) {
       rowWidgets.add(
          Padding(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  var rack = store['data'][alphabet]?.firstWhere(
                  ...
                  Navigator.push<void>(
                    ...
                    MaterialPageRoute<void>(
                      builder: (BuildContext context) => ProductPage(p_id: productId),
                    ),
                  );
                },
                style: ButtonStyle(
